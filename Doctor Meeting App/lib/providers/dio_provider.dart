...
class DioProvider {
  Future<dynamic> getToken(
    ...
    try {
      var response = await Dio().post(
        'http://192.168.1.14:8000/api/login',
        data: {
          'email': email,
          'password': password,
        },
  ...
  Future<dynamic> getUser(String token) async {
    try {
      var user = await Dio().get(
        'http://192.168.1.14:8000/api/user',
  ...
  Future<dynamic> bookAppointment(
    ...
    try {
      var response = await Dio().post(
        'http://192.168.1.14:8000/api/book',
        data: {
          'date': date,
          'day': day,
          'time': time,
  ...
  Future<dynamic> getAppointments(String token) async {
    try {
      var response = await Dio().get(
        'http://192.168.1.14:8000/api/appointments',
        options: Options(headers: {'Authorization': 'Bearer $token'}),
      );
      if (response.statusCode == 200 && response.data != '') {
  ...
  Future<dynamic> storeReviews(
    ...
    try {
      var response = await Dio().post(
        'http://192.168.1.14:8000/api/reviews',
        data: {
          'ratings': ratings,
          'reviews': reviews,
          'appointment_id': id,
  ...
  Future<dynamic> storeFavDoc(String token, List<dynamic> favList) async {
    try {
      var response = await Dio().post(
        'http://192.168.1.14:8000/api/fav',
